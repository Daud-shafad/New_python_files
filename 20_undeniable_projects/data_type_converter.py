...
# warning: eval() built-in function is generally unsafe, but useful for quick type demo
...
while True:
    ...
    if user_choice == '9':
       ...
       match user_choice:
        ...
        case '1':
            user_type = input("Enter the type you're converting (float): ")
            data = eval(user_type)
            if type(data) != int: 
               print(f"converting {data} to integer: {int(data)}") 
            else:
                print("You already have an integer type")
        
        case '2':
            user_type = input("Enter the type you're converting (int): ")
            data = eval(user_type)
            if type(data) != float: 
               print(f"converting {data} to float: {float(data)}") 
            else:
                print("You already have a float type")
        
        case '3':
            user_type = input("Enter the type you're converting (int/float): ")
            data = eval(user_type)
            if type(data) != complex: 
               print(f"converting {data} to complex: {complex(data)}") 
            else:
