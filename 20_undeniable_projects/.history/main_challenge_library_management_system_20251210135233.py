        ...
        match user_choice:
            ...
            case '4':
                ...
                if member_id in members_dict and book_id not in books_dict:
                    books_dict[book_id].update(book_id)
            ...
            case '5':
                user_searchType = input("Enter the type of your search (1.title  2.author  3.genre): ")
                search_book = input("Enter The Title/Author/Genre of the Book: ")
                
                match search_book:

                  case title if title in books_dict:
                       books_dict.get(title)
                       result_search = f"The book you want is: {books_dict[book_id]}"
                       print(result_search)
                  
                  case author if author in books_dict:
                       books_dict.get(author)
                       result_search = f"The book you want is: {books_dict[book_id]}"
                       print(result_search)

                  case genre if genre in books_dict:
                       books_dict.get(genre)
                       result_search = f"The book you want is: {books_dict[book_id]}"
                       print(result_search)
            ...
            case '6':
                member_id = int(input("Enter member ID: "))
                if member_id in members_dict and borrowed_books in members_dict:
                   print(f"{borrowed_books}" in f"{members_dict[member_id]}")
