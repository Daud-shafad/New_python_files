...
while True:
    ...
    if user_choice == '4':
        ...
        match user_choice:
            ...
            case '2': #in this case it is very important to understand it, 
                #in prime number when the (while divisor * divisor <= user_number:) 
                #condition becomes false, it becomes prime, while in composite number when the
                # condition (if user_number % divisor == 0:) becomes true, it becomes composite 
                user_number = int(input("Enter a number: "))
                is_prime = 1
                divisor = 2
                if user_number <= 1:
